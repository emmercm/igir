  ...
  it('should copy and test', async () => {
    await copyFixturesToTemp(async (inputTemp, outputTemp) => {
      const result = await runIgir({
        ...
        inputExclude: [path.join(inputTemp, 'roms', 'discs')], // test disc scanning + matching
        output: outputTemp,
        dirDatName: true,
  ...
  it('should symlink and test', async () => {
    await copyFixturesToTemp(async (inputTemp, outputTemp) => {
      ...
      expect(result.outputFilesAndCrcs).toEqual([
        ...
        [`${path.join('One', 'UMD.iso')} -> ${path.join('<input>', 'discs', 'UMD.iso')}`, 'e90f7cf5'],
  ...
  it('should relative symlink, and test', async () => {
    await copyFixturesToTemp(async (inputTemp, outputTemp) => {
      ...
      expect(result.outputFilesAndCrcs).toEqual([
        ...
        [`UMD.iso -> ${path.join('..', 'input', 'roms', 'discs', 'UMD.iso')}`, 'e90f7cf5'],
