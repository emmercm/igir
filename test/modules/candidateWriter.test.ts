...
async function copyFixturesToTemp(
  ...
  const inputTemp = await fsPoly.mkdtemp(path.join(Defaults.GLOBAL_TEMP_DIR, 'input'));
  await fsPoly.copyDir('./test/fixtures', inputTemp);

  // Set up the output directory, but delete it so ROMWriter can make it
  const outputTemp = await fsPoly.mkdtemp(path.join(Defaults.GLOBAL_TEMP_DIR, 'output'));
