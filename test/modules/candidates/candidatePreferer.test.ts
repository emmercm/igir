...
async function expectFilteredCandidates(
  ...
  const [filteredParentsToCandidates] = await Promise.all([
    buildCandidateFilter(options).prefer(dat, new Map(parentsToCandidates)),
  ]);

  const totalCandidates = [...filteredParentsToCandidates.values()].reduce(
...
function expectPreferredCandidates(
  ...
  const filteredParentsToCandidates = buildCandidateFilter(options).prefer(
    dat,
    new Map(parentsToCandidates),
  );
  ...
  const candidateNames = [...filteredParentsToCandidates.values()].flatMap((candidate) =>
    candidate.map((c) => c.getName()),
  );
          ...
          new ReleaseCandidate(
            ...
            await Promise.all(
              game
                ...
                .map(
                  async (gameRom) =>
                    new ROMWithFiles(gameRom, await gameRom.toFile(), await gameRom.toFile()),
                ),
            ),
          ),
        );
...
describe('sort', () => {
  describe('prefer game regex', () => {
    it('should return the first candidate when option is empty', async () => {
      expectPreferredCandidates(
        { preferGameRegex: undefined, single: true },
        [
    ...
    it('should return the first candidate when none matching', async () => {
      expectPreferredCandidates(
        { preferGameRegex: 'NINE', single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['two', 'three'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['four', 'five', 'six'], [], 'EN'),
    ...
    it('should return the first matching candidate when some matching', async () => {
      expectPreferredCandidates(
        { preferGameRegex: '/THREE|five/i', single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['two', 'three'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['four', 'five', 'six'], [], 'EN'),
    ...
    it('should return the first candidate when all matching', async () => {
      expectPreferredCandidates(
        { preferGameRegex: '[aeiou]', single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['two', 'three'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['four', 'five', 'six'], [], 'EN'),
  ...
  describe('prefer rom regex', () => {
    it('should return the first candidate when option is empty', async () => {
      expectPreferredCandidates(
        { preferRomRegex: undefined, single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['two', 'three'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['four', 'five', 'six'], [], 'EN'),
    ...
    it('should return the first candidate when none matching', async () => {
      expectPreferredCandidates(
        { preferRomRegex: '/five\\.nes/i', single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['two', 'three'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['four', 'five', 'six'], [], 'EN'),
    ...
    it('should return the first matching candidate when some matching', async () => {
      expectPreferredCandidates(
        { preferRomRegex: '/THREE|five\\.rom/i', single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['two', 'three'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['four', 'five', 'six'], [], 'EN'),
    ...
    it('should return the first candidate when all matching', async () => {
      expectPreferredCandidates(
        { preferRomRegex: '[aeiou]', single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['two', 'three'], [], 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['four', 'five', 'six'], [], 'EN'),
  ...
  describe('prefer verified', () => {
    it('should return the first candidate when option is false', async () => {
      expectPreferredCandidates(
        { preferVerified: false, single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], [], 'EN'),
    ...
    it('should return the first candidate when none matching', async () => {
      expectPreferredCandidates(
        { preferVerified: true, single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], [], 'EN'),
  ...
  describe('prefer languages', () => {
    ...
    it('should return the first matching candidate when some matching', async () => {
      expectPreferredCandidates(
        { preferLanguage: ['EN', 'JA'], single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage('one', 'USA', 'EN'),
          await buildReleaseCandidatesWithRegionLanguage('two', 'USA', ['ES', 'EN']),
          await buildReleaseCandidatesWithRegionLanguage('three', 'JPN', 'JA'),
    ...
    it('should treat "World" as English', () => {
      const gameParent = new Game({
        name: 'Akumajou Special - Boku Dracula-kun (Japan)',
        release: new Release('Akumajou Special - Boku Dracula-kun (Japan)', 'JPN'),
      });
      const gameWorldJa = new Game({
        name: 'Akumajou Special - Boku Dracula-kun (World) (Ja) (Castlevania Anniversary Collection)',
      });
      const gameWorld = new Game({
        name: 'Kid Dracula (World) (Castlevania Anniversary Collection)',
      });
      const games = [gameParent, gameWorldJa, gameWorld];
      const parent = new Parent(gameParent, games);
      const releaseCandidates = games.map(
        (game) => new ReleaseCandidate(game, game.getReleases()[0], []),
      );
      expectPreferredCandidates(
        { single: true, preferLanguage: ['EN'] },
    ...
    test.each([
      ...
      'should rank candidates by all preferred languages: %s',
      (gameNames, preferLanguage, expectedName) => {
  ...
  describe('prefer regions', () => {
    ...
    it('should return the first candidate when none matching', async () => {
      expectPreferredCandidates(
        { preferRegion: ['USA', 'EUR'], single: true },
        [
    ...
    test.each([
      ...
      'should return the first candidate when all matching by name: %s',
      async (names, expectedName) => {
        expectPreferredCandidates(
  ...
  describe('prefer revision newer', () => {
    ...
    test.each([
      ...
      'should return the first matching candidate when some matching: %s',
      async (names, expectedName) => {
        expectPreferredCandidates(
          { preferRevision: PreferRevision[PreferRevision.NEWER].toLowerCase(), single: true },
          [await buildReleaseCandidatesWithRegionLanguage(names)],
  ...
  describe('prefer retail', () => {
    it('should return the first candidate when option is false', async () => {
      expectPreferredCandidates(
        { preferRetail: false, single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage(['one'], 'USA', 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['two (Aftermarket)', 'two'], 'USA', 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['three [b]', 'three'], 'USA', 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(
            ['four (Beta)', 'four (Proto)', 'four'],
            'USA',
            'EN',
          ),
          await buildReleaseCandidatesWithRegionLanguage(
            ['five (Demo)', 'five', 'five (Sample)'],
            'USA',
            'EN',
          ),
  ...
  describe('prefer parent', () => {
    it('should return the first candidate when option is false', async () => {
      expectPreferredCandidates(
        { preferParent: false, single: true },
        [
          await buildReleaseCandidatesWithRegionLanguage('one', 'USA', 'EN'),
          await buildReleaseCandidatesWithRegionLanguage(['two', 'two two'], 'USA', 'EN'),
          await buildReleaseCandidatesWithRegionLanguage('three', 'USA', 'EN', { cloneOf: 'zero' }),
          await buildReleaseCandidatesWithRegionLanguage(
            ['four (Parent)', 'four (Clone)'],
            'USA',
            'EN',
            [{}, { cloneOf: 'zero' }],
          ),
          await buildReleaseCandidatesWithRegionLanguage(
            ['five (Clone)', 'five (Parent)'],
            'USA',
            'EN',
            [{ cloneOf: 'zero' }, {}],
          ),
          await buildReleaseCandidatesWithRegionLanguage(
            ['six (Clone 1)', 'six (Clone 2)'],
            'USA',
            'EN',
            [{ cloneOf: 'zero' }, { cloneOf: 'zero' }],
          ),
        ],
        [
          'one (USA) (EN)',
    ...
    it('should return the first candidate when all matching', async () => {
      expectPreferredCandidates(
        ...
        [
          ...
          await buildReleaseCandidatesWithRegionLanguage(
            ['two (Parent)', 'two (Clone)'],
            'USA',
            'EN',
            [{}, { cloneOf: 'zero' }],
          ),
          await buildReleaseCandidatesWithRegionLanguage(
            ['three (Clone)', 'three (Parent)'],
            'USA',
            'EN',
            [{ cloneOf: 'zero' }, {}],
          ),
          await buildReleaseCandidatesWithRegionLanguage(
            ['four (Clone 1)', 'four (Clone 2)'],
            'USA',
            'EN',
            [{ cloneOf: 'zero' }, { cloneOf: 'zero' }],
          ),
