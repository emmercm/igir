...
Unlike traditional console emulators that only have to emulate a small set of hardware, arcade emulators have the monumental task of emulating a wildly varying number of hardware chips, inputs, and outputs. This means that each version of an emulator has a very specific set of games it can emulate, and each emulator version will likely add more games to this set.

Due to arcade machines being more complicated and fewer, arcade ROM dumps are sometimes imperfect. Because of this, newer emulator versions may expect different ROM files than older versions. This makes ROM sets potentially incompatible between different emulator versions.
...
!!! danger
    ...
    tl;dr each arcade emulator version has an _exact_ DAT you need to use or your games may not work.
...
5. **Determine what kind of ROM set we want.**
    ...
    Let's say we care first and foremost that the arcade games "just work," and then would like to conserve disk space. A "split" ROM set makes a good choice because RetroArch should be able to automatically index every game, including both parents and clones.
