...
Input options (supports globbing):
  ...
  -p, --patch          Path(s) to ROM patch files or archives (supported: .aps, .bps, .dps, .e
                       bp, .ips, .ips32, .ppf, .rup, .ups, .vcdiff, .xdelta)           [array]
...
Advanced usage:
  ...
  Tokens that are replaced when generating the output (--output) path of a ROM:
    ...
    {es}        The ROM's emulator-specific /roms/* directory for EmulationStation (e.g. "gb")
