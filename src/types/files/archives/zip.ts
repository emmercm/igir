  ...
  async getArchiveEntries(checksumBitmask: number): Promise<ArchiveEntry<Zip>[]> {
    ...
    return async.mapLimit(
      ...
      async (entryFile, callback: AsyncResultCallback<ArchiveEntry<Zip>, Error>) => {
        ...
        const additionalChecksumBitmask = checksumBitmask & ~ChecksumBitmask.CRC32;
        if (additionalChecksumBitmask) {
          const entryStream = entryFile.stream()
            ...
            .on('error', () => {});
          try {
            checksumsWithoutCrc = await FileChecksums
              .hashStream(entryStream, additionalChecksumBitmask);
          } finally {
            /**
